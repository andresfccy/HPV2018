    ...
    public partial class UsuariosSistema : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                String rutaPlantilla = String.Format("{0}/usuariosSistema.xlsx", pathPlantilla);
                String rutaRpt = String.Format("{0}/usuariosSistema.xlsx", pathTmp);

                ManejadorExcel rpt = new ManejadorExcel();

                rpt.Abrir(rutaPlantilla);

                OE_RptUsuariosSistema oe = new OE_RptUsuariosSistema();
                ...
                OS_RptUsuariosSistema os = new FachadaReporte().RptUsuariosSistema(oe);

                if (os.Respuesta.Codigo == 0)
                {
                    Int32 fila = 10;
                    bool primeraFila = true;
                    foreach (HPV_Entidades.Reporte.UsuariosSistema reg in os.ListaUsuariosSistema)
                    {
                        Int32 colum = 2;
                        var i = 0;
                        if (primeraFila)
                ...
                Response.ContentType = "application/vnd.ms-excel";
                    Response.AddHeader("Content-Disposition", "attachment;filename=UsuariosSistema-p" + idPeriodo + "-" + FechaCorte + ".xlsx");
                Response.Charset = "";
