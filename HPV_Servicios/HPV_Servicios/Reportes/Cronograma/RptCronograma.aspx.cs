...
namespace HPV_Servicios.Reportes.Cronograma
{
    public partial class RptCronograma : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                String rutaPlantilla = String.Format("{0}/cronogramaTaller.xlsx", pathPlantilla);
                String rutaRpt = String.Format("{0}/cronogramaTaller.xlsx", pathTmp);

                ManejadorExcel rpt = new ManejadorExcel();

                rpt.Abrir(rutaPlantilla);

                OE_RptCronograma oe = new OE_RptCronograma();
                ...
                OS_RptCronograma os = new FachadaReporte().RptCronogramaTaller(oe);

                if (os.Respuesta.Codigo == 0)
                {
                    Int32 fila = 13;
                    bool primeraFila = true;
                    foreach (HPV_Entidades.Reporte.CronogramaTaller reg in os.ListaCronograma)
                    {
                        Int32 colum = 2;
                        var i = 0;
                        if (primeraFila)
