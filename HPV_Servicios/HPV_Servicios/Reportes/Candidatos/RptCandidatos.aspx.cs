...
namespace HPV_Servicios.Reportes.Candidatos
{
    public partial class RptCandidatos : System.Web.UI.Page
    {
        protected void Page_Load(object sender, EventArgs e)
        {
            try
            {
                ...
                String rutaPlantilla = String.Format("{0}/candidatos.xlsx", pathPlantilla);
                String rutaRpt = String.Format("{0}/candidatos.xlsx", pathTmp);

                ManejadorExcel rpt = new ManejadorExcel();

                rpt.Abrir(rutaPlantilla);

                OE_RptCandidatos oe = new OE_RptCandidatos();
                ...
                OS_RptCandidatos os = new FachadaReporte().RptCandidatos(oe);

                if (os.Respuesta.Codigo == 0)
                {
                    Int32 fila = 10;
                    bool primeraFila = true;
                    foreach (HPV_Entidades.Reporte.Candidatos reg in os.ListaCandidatos)
                    {
                        Int32 colum = 2;
                        var i = 0;
                        if (primeraFila)
