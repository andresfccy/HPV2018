...
var modSigs = [require('../signatures/module')].concat(require('../signatures/simple'));
var refSigs = [require('../signatures/assign'), require('../signatures/decl')];
...
markPass.run = function(ast, info) {
  ...
  var findRefs = function() {
    ...
    deepApply(ast, refSigs, function(chunk) {
      var id = chunk.id ? chunk.id.name : chunk.expression.left.name;
