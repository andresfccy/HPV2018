...
gulp.task('bump-version', function() {
  return gulp.src(['./bower.json', './package.json']).pipe(bump({type: "patch"}).on('error', gutil.log)).pipe(gulp.dest('./'));
});
...
gulp.task('release', ['test'], function(callback) {
  runSequence('bump-version', 'build', 'commit-changes', 'tag-version', function(error) {
    if (error) {
...
gulp.task('build', function() {
  return gulp.src("src/angular-ui-router-title.js").pipe(wrap({src: './build.txt'}, {info: require('./package.json!systemjs-json')})).pipe(gulp.dest('.'));
});
