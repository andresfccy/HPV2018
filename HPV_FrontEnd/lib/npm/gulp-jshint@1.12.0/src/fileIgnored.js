...
(function(process) {
  ...
  var ignoreLoader = new RcLoader('.jshintignore', {}, {loader: function(path) {
      var contents = fs.readFileSync(path, 'utf8');
      var dir = dirname(path);
      return {
        file: path,
        patterns: contents.toString('utf8').split(/\r?\n/).filter(function(line) {
          return !!line.trim();
        }).map(function(line) {
  ...
  module.exports = (function() {
    ...
    return function check(file, cb) {
      if (file.isNull())
        return cb(null, true);
      if (file.isStream())
        return cb(new PluginError('gulp-jshint', 'Streaming not supported'));
      var path = file.path;
