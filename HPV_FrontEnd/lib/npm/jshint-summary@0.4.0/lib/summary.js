...
var reporter = exports.reporter = function(result, data, options) {
  ...
  var lines = [];
  var table;
  var wordwrapper = options.wordWrap ? wordwrap(0, options.width) : function(s) {
    return s;
  };
  ...
  result.forEach(function(item) {
    ...
    var err = item.error;
    var line = [brush.code(err.code), brush.position('Line ' + err.line + ', Col ' + err.character), brush.reason(err.reason)];
    table.push(line);
  });
  if (table) {
  ...
  if (result.length > 0) {
    var summ = [symbols.cross, result.length, utils.pluralise('problem', result.length), 'in', fileCount, utils.pluralise('file', fileCount)];
  ...
  if (options.statistics) {
    data.forEach(function(data) {
      if (data.implieds || data.unused || data.globals) {
        ...
        table = utils.createTable(options);
        if (data.globals) {
          table.push({'Module globals:': brush.variable(data.globals.sort().join(', '))});
        }
        if (data.implieds) {
          table.push({'Implied globals:': data.implieds.map(function(global) {
              return brush.variable(global.name) + ' @ line ' + global.line;
            }).join(', ')});
        }
        if (data.unused) {
          table.push({'Unused variables:': data.unused.map(function(unused) {
