...
(function(process) {
  ...
  gulp.task('lint', function() {
    return gulp.src([__filename, './index.js', './test/**/test_*.js']).pipe(jshint()).pipe(map(function(file, cb) {
  ...
  gulp.task('default', ['lint'], function() {
    if (jshintErrors.length) {
      console.error(jshintErrors.map(function(error) {
        return util.format('[%s] %s in (%s:%d)\n', colors.green('gulp-jshint'), colors.red(error.error.reason), error.file, error.error.line);
      }).join(''));
      process.exit(1);
    }
  });
