...
exports.tests = {
  ...
  'should report errors when indentation is incorrect': function(test) {
    ...
    expected[file] = {
      '3': [merge({}, Messages.INDENTATION_SPACES_AMOUNT, {
        message: Messages.INDENTATION_SPACES_AMOUNT.message.replace('{a}', 4).replace('{b}', 5),
        line: 3,
        payload: {
          expected: 4,
          indent: 5
      ...
      '5': [merge({}, Messages.INDENTATION_SPACES_AMOUNT, {
        message: Messages.INDENTATION_SPACES_AMOUNT.message.replace('{a}', 12).replace('{b}', 10),
        line: 5,
        payload: {
          expected: 12,
          indent: 10
  ...
  'should report an error when file with BOM is not allowed': function(test) {
    ...
    report = validator.getInvalidFiles();
    expected = {};
    expected[file] = {'1': [merge({}, Messages.INDENTATION_SPACES, {line: 1})]};
    test.deepEqual(report, expected);
    test.done();
  }
};
